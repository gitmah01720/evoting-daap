...
export const VotingProvider = ({ children }) => {
  ...
  const createVoter = async (formInput, fileUrl, router) => {
    try {
      ...
      console.log(
        `Name = ${name}, adr = ${address} ,pos = ${position} url = ${fileUrl}`
      );
  ...
  const giveVote = async (id) => {
    try {
      ...
      const votedList = await contract.doVote(candiAddress, candiId);
  ...
  const createCandidate = async (formInput, fileUrl, router) => {
    try {
      ...
      const candi = await contract.setCandidate(
        address,
        name,
        age,
        fileUrl,
